...
| `dr` | real | Curvature `d^T r = s^T y / stp` (caller validated `dr > 0`). |
| `rr` | nonneg real | `||r||^2 = y^T y`. |
| `dtd` | nonneg real | `||d||^2 = d^T d`. |
| `ws`, `wy` (in/out) | real matrices `n * m` | History columns. The column at `itail` is overwritten. |
| `sy`, `ss` (in/out) | real matrices `m * m` | Cached `S^TY` / `S^TS` (lower / upper packing -- see `01_glossary.md`). |
...
- `dr > 0`. The caller (`mainlb`) checks the curvature condition
  `s^T y > eps * ||y||^2` *before* calling `matupd`; `dr` here is
  `s^T y / stp`, and the caller's check ensures the corresponding
  `s^T y` is positive.
...
- Caller checks `s^T y > eps * ||y||^2` and skips `matupd` if violated;
