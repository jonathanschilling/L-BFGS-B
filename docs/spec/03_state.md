...
| `theta` | real, `> 0` | Hessian scaling (`y_k^T y_k / s_k^T y_k` after each update; initialized to `1`). |
...
|--------|------|------------|
| `sy` | real matrix, `col * col` | `sy[i,j] = s_i^T * y_j`. The diagonal is `D = diag(s_i^T y_i)`. The strict lower triangle is `L_{i,j} = s_i^T y_j` for `i > j`. The strict upper triangle is unused (in the F77 packing). |
| `ss` | real matrix, `col * col` | `ss[i,j] = s_i^T * s_j`. Symmetric; the F77 source stores only the upper triangle. |
| `T` | real matrix, `col * col`, upper-triangular | Cholesky factor: `T^T T = theta * S^TS + L * D^{-1} * L^T` (the Cholesky factor of the middle matrix `M_inv`'s lower-right block). Computed by `formt` from `sy`, `ss`, `theta`. |
...
| `c` | real vector, length `2 col` | `c = W^T (xc - x)`; built by `cauchy`, used by `subsm`. |
...
1. **Check curvature**: compute `s = x_new - x` and `y = g_new - g`. If
   `s^T y / |s^T s| > eps * ||y||^2` (a refresh threshold; see `mainlb.md`
   for the exact formula), accept the pair.
2. **If accepted**: append `(s, y)` to `S`, `Y`. If `col = m` already,
   drop the oldest column first. Update `theta = (y^T y) / (s^T y)`.
