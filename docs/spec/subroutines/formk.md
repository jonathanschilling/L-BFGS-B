...
Form the **`LEL^T`-factorization** of the `2col * 2col` indefinite
...
`L_a` is the strict-lower triangle of `S^TAA^TY`, and `R_z` is the
upper triangle of `S^TZZ^TY`. (See `code.pdf` sec.4 and Byrd/Lu/Nocedal/Zhu
1995 sec.5.)
...
| Name | Type | Description |
|------|------|-------------|
| `wn` | real matrix, `2m * 2m` | Upper triangle holds the `LEL^T` factorization. |
...
- On success: `wn[1..2col, 1..2col]` upper triangle is the `LEL^T`
...
`Y^TZZ^TY` contribution wasn't in there before) and subtract from the
(2,2) block (their `S^TAA^TS` contribution was). Variables that *left*
have the opposite.
