...
driver). The algorithm follows More & Thuente 1994 sec.3 (case 1-4).
...
- **Case 2 -- Lower `f`, opposite-sign derivatives**: `fp <= fx and sgnd < 0`.
  ...
  `stp`. **Set `brackt = true`.**
- **Case 3 -- Lower `f`, same sign, magnitude of `f^T` decreases**:
...
- **Case 4 -- Lower `f`, same sign, magnitude of `f^T` does not decrease**:
...
|----------|-----|-------|---------|
| `p66` | `p66` | `0.66d0` | Bracket-safeguard multiplier. From More-Thuente 1994 sec.3.5. |
...
- **Paper**: More & Thuente 1994 sec.3 (the algorithm). MINPACK-1 (1983)
